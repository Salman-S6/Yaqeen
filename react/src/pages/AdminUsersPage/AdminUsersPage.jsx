...
const AdminUsersPage = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            if (activeTab === 'team') {
                ...
                setEmployees(getResponseCollection(response));
            } else if (activeTab === 'citizens') {
                const response = await employeeService.getCitizens();
                setCitizens(getResponseCollection(response));
            }
        } catch (error) {
            showNotification(getApiErrorMessage(error, "فشل الاتصال بالخادم الرئيسي."), 'error');
    ...
    const handleToggleCitizenStatus = async (id) => {
        try {
            ...
            showNotification(getApiErrorMessage(error, "حدث خطأ أثناء تغيير حالة المواطن"), "error");
        }
    };
    ...
    const handleViewCitizenDetails = async (id) => {
        try {
            ...
            setSelectedCitizenDetails(getResponseData(response));
            setIsCitizenModalOpen(true);
        } catch (error) {
            console.error("فشل جلب تفاصيل المواطن:", error);
            showNotification(getApiErrorMessage(error, "فشل جلب تفاصيل المواطن."), "error");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSaveChanges = async (e) => {
        ...
        try {
            ...
            if (isAddMode) {
                ...
                setEmployees(prev => [...prev, { ...created, ...payload, status: payload.status }]);
                ...
                setEmployees(prev => prev.map(emp => emp.id === selectedEmployee.id ? { ...emp, ...updated, ...payload, status: payload.status } : emp));
                showNotification(response.data?.message || "تم تحديث بيانات الموظف بنجاح.", 'success');
            }
            setIsModalOpen(false); resetForm();
        } catch (error) {
            showNotification(getApiErrorMessage(error, "فشل التحديث، تفقد المدخلات."), 'error');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const executeDeleteEmployee = async () => {
        ...
        } catch (error) { showNotification(getApiErrorMessage(error, "فشل الحذف."), 'error'); }
        ...
        <div className={styles.pageContainer}>
            <div className={styles.mainContentCard}>
                <div className={styles.pageTitleSection}>
                    {activeTab === 'team' && <button type="button" className={styles.addEmployeeBtn} onClick={handleOpenAddModal}><FaUserPlus /> إضافة موظف جديد</button>}
                    <div className={styles.titleInfo}><h2>إدارة المستخدمين</h2><p>إدارة صلاحيات الموظفين وسجلات المواطنين في النظام</p></div>
                </div>

                <div className={styles.tabsContainer}>
                    <button type="button" className={`${styles.tabBtn} ${activeTab === 'citizens' ? styles.activeTab : ''}`} onClick={() => setActiveTab('citizens')}>سجل المواطنين</button>
                    <button type="button" className={`${styles.tabBtn} ${activeTab === 'team' ? styles.activeTab : ''}`} onClick={() => setActiveTab('team')}>الفريق الوظيفي</button>
                </div>
                                            ...
                                            <tr key={emp.id}>
                                                ...
                                                <td>
                                                    <div className={styles.actionsContainer}>
                                                        ...
                                                        <button type="button" className={styles.statusToggleBtn} onClick={() => handleOpenEditModal(emp)}>تعديل</button>
                                                        <button type="button" className={styles.deleteUserBtn} onClick={() => setConfirmDelete({ isOpen: true, employeeId: emp.id })}><FaTrash /></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
                    <div style={{ backgroundColor: '#fff', width: '600px', borderRadius: '12px', overflow: 'hidden', direction: 'rtl', maxHeight: '90vh', overflowY: 'auto' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '15px 20px', borderBottom: '1px solid #e5e7eb', backgroundColor: '#f9fafb' }}>
                            ...
                            <button type="button" onClick={() => setIsCitizenModalOpen(false)} style={{ background: 'none', border: 'none', fontSize: '18px', cursor: 'pointer' }}><FaTimes /></button>
                        </div>
